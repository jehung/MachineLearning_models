...
def fit_dt(train=None, target=None, size=0):
...
if  __name__== '__main__':
    ...
    df = Parallel(n_jobs=6)(delayed(fit_dt)(train=train, target=target, size=size) for size in [0.4, 0.5, 0.6, 0.7, 0.8, 0.9])
    merge_dict(df)
    #pool = multiprocessing.Pool(processes=6)
    #df = pool.starmap(fit_dt, zip(repeat(train, target), range(0.4,0.6,0.1)))
    print(df)
